...
def run_stage_to_dp(time_range: str):
    df_stage = pandas_gbq.read_gbq(f"""SELECT user, query_timestamp, table_uri, service_account, metabase, intra_team, workload_identity, dataproduct, source, target, dataproduct_id
